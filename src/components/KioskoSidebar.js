...
const KioskoSidebar = ({ activeTab, setActiveTab, isAdmin, isMobileOpen, onClose }) => {
  ...
  const menuItems = [
    ...
    {
      id: 'sales-report', label: 'Reporte de Ventas', icon: (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 mr-3">
  ...
  return (
    <>
      ...
      {isMobileOpen && (
        <div
          className="fixed inset-0 bg-slate-900 bg-opacity-50 z-40 md:hidden backdrop-blur-sm"
          onClick={onClose}
        />
      )}
      ...
      <div className={`
        ...
        w-72 bg-white border-r border-slate-200 h-screen flex flex-col
      `}>
        {/* Encabezado del sidebar */}
        <div className="p-6 border-b border-slate-100 flex items-center justify-center">
          <h2 className="text-2xl font-bold text-primary-600 tracking-tight">ATUCSARA</h2>
        </div>
          ...
          <ul className="space-y-1">
            {menuItems.map((item) => (
              <li key={item.id}>
                <button
                  onClick={() => {
                    console.log('Sidebar clicked:', item.id); // DEBUG LOG
        ...
        <div className="p-4 border-t border-slate-100">
          <button
            ...
            className="w-full flex items-center justify-center px-4 py-2.5 text-red-600 hover:bg-red-50 hover:text-red-700 rounded-lg text-sm font-medium transition-colors border border-transparent hover:border-red-100"
          >
